...
Không còn flag mức độ, không còn toggle `--wow`. `vdesign <target>` là mode duy nhất — audit-driven: sửa mọi thứ Phase 2 tìm ra, bất kể độ sâu (spacing → cấu trúc → đổi hướng thị giác), và mọi lần chạy đều mở đầu bằng Aesthetic Customization ở Phase 0, nơi user chọn tường minh 18 tiêu chí phong cách/cấu trúc (typography, màu, motion, shape, layout, tự do cấu trúc, độ táo bạo, và nhiều hơn nữa — xem `references/aesthetic-customization.vi.md`) thay vì để 1 flag âm thầm mở khóa hay kìm hãm sáng tạo. Không còn trần chặn, không còn kiểu "tìm ra rồi nhưng không fix được vì sai mức".
...
7. **Domain Research** — có điều kiện, chỉ chạy khi tiêu chí Aesthetic Boldness ở step 6 trả lời *Trend-forward*, *Experimental/bold*, hoặc *Game-inspired/Arcade*, hoặc câu trả lời khác tường minh gọi tới trào lưu thật hiện hành; *Brand-centric* và *Functional/minimalist* bỏ qua thẳng tới step 8:
   ...
   - Khi tiêu chí 14 = *Game-inspired/Arcade*: ưu tiên nguồn chuyển sang gallery chuyên game UI (Dribbble tag "game UI", GameUIDatabase.com, Behance HUD design) thay vì Awwwards — category Game của Awwwards khá mỏng so với các category thường
...
8. **Design Brief** — trước khi đụng vào code: tổng hợp 18 câu trả lời ở step 6 cộng ghi chú vét ở Vòng 6, cộng kết quả step 7 nếu có chạy, thành 1 brief cụ thể 5-10 dòng (xem section Design Brief trong `references/aesthetic-customization.vi.md`) — tên typeface thật, cách tiếp cận bảng màu thật, timing motion thật, hướng shape/icon/imagery thật, spacing scale thật, hệ thống layout thật, tone thật, mức độ táo bạo thật, và (khi áp dụng) cách xử lý data/table, form, empty/loading/error thật. Một nhãn-một-từ cho mỗi tiêu chí không phải là brief. Nói rõ ra, và ghi 18 câu trả lời + Design Brief đã tổng hợp vào `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md`, trước khi Phase 1 bắt đầu, nêu trạng thái của step 7 nếu có chạy (cached/fresh/fallback/unavailable/skipped). Phase 3 đọc lại file này trước khi thực thi, thay vì dựa vào trí nhớ hội thoại. Một lần chạy không tổng hợp (18 câu trả lời thô, chưa chốt brief) là lý do lớn nhất khiến output vẫn đọc ra generic/an toàn.
...
3. Trình cụm pilot cho user duyệt trước khi đụng tới bất kỳ component nào khác trong inventory: có URL/localhost → chụp screenshot, show before/after; không có → mô tả cách xử lý cụ thể (màu, spacing, motion) và show diff code của riêng cụm pilot. Hỏi qua `AskUserQuestion`: duyệt / chỉnh sửa (user nêu rõ muốn đổi gì) / thử hướng khác.
4. Nếu chỉnh sửa hoặc đổi hướng: làm lại pilot theo feedback đó, trình lại. Cap 3 vòng (tái dùng đúng số review-cycle của vreview) — quá 3 vòng thì hỏi "duyệt kèm bất đồng đã nêu, hay dừng lại".
...
**Đổi ý giữa chừng cho 1 tiêu chí.** Nếu khi chạy Phase 3 phát hiện 1 câu trả lời tiêu chí ở Phase 0 không còn phù hợp (vd đã chốt *Giữ token hiện có* nhưng giờ có gap thật cần thêm accent màu mới), đừng restart lại toàn bộ Aesthetic Customization. Hỏi đúng 1 `AskUserQuestion` cho riêng tiêu chí đó (giữ nguyên options, đánh dấu rõ đáp án hiện tại), cập nhật lại dòng tổng hợp của tiêu chí đó trong file Design Brief, rồi tiếp tục Phase 3 từ chỗ đang dừng. Cap 3 lần đổi ý/run (tái dùng đúng cap của bước duyệt pilot cluster ở trên) — quá 3 lần thì hỏi user: khoá Design Brief hiện tại và tiếp tục, hay dừng run để re-plan.
