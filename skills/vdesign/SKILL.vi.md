...
Không còn flag mức độ, không còn toggle `--wow`. `vdesign <target>` là mode duy nhất — audit-driven: sửa mọi thứ Phase 2 tìm ra, bất kể độ sâu (spacing → cấu trúc → đổi hướng thị giác), và mọi lần chạy đều mở đầu bằng Aesthetic Customization ở Phase 0, nơi user chọn tường minh 17 tiêu chí phong cách (typography, màu, motion, shape, layout, độ táo bạo, và nhiều hơn nữa — xem `references/aesthetic-customization.vi.md`) thay vì để 1 flag âm thầm mở khóa hay kìm hãm sáng tạo. Không còn trần chặn, không còn kiểu "tìm ra rồi nhưng không fix được vì sai mức".
...
6. **Aesthetic Customization** — luôn luôn, mọi lần chạy, không cần flag: đọc toàn bộ `references/aesthetic-customization.vi.md` và chạy đủ 5 vòng `AskUserQuestion` của nó (17 tiêu chí — typography, màu, motion, elevation, background, shape, icon, imagery, spacing, grid, layout columns, dark mode, brand tone, độ táo bạo tổng thể, mật độ data/table, style form/input, style empty/loading/error state). User chọn tường minh, mỗi lần chạy; agent không bao giờ tự suy đoán hướng đi.
...
8. **Design Brief** — trước khi đụng vào code: tổng hợp 17 câu trả lời ở step 6, cộng kết quả step 7 nếu có chạy, thành 1 brief cụ thể 5-10 dòng (xem section Design Brief trong `references/aesthetic-customization.vi.md`) — tên typeface thật, cách tiếp cận bảng màu thật, timing motion thật, hướng shape/icon/imagery thật, spacing scale thật, hệ thống layout thật, tone thật, mức độ táo bạo thật, và (khi áp dụng) cách xử lý data/table, form, empty/loading/error thật. Một nhãn-một-từ cho mỗi tiêu chí không phải là brief. Nói rõ ra, và ghi 17 câu trả lời + Design Brief đã tổng hợp vào `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md`, trước khi Phase 1 bắt đầu, nêu trạng thái của step 7 nếu có chạy (cached/fresh/fallback/unavailable/skipped). Phase 3 đọc lại file này trước khi thực thi, thay vì dựa vào trí nhớ hội thoại. Một lần chạy không tổng hợp (17 câu trả lời thô, chưa chốt brief) là lý do lớn nhất khiến output vẫn đọc ra generic/an toàn.
...
- ❌ Bỏ qua Aesthetic Customization ở Phase 0 hoặc coi 17 câu trả lời của nó là chi tiết trang trí thay vì tổng hợp thành Design Brief thật (step 8) trước khi đụng code — 1 lần chạy không có điểm neo vẫn đọc ra là generic/an toàn; chọn pattern trước khi chọn hướng chỉ tạo ra một mớ hổ lốn, không phải một thiết kế mạch lạc
