...
Không còn flag mức độ, không còn toggle `--wow`. `vdesign <target>` là mode duy nhất — audit-driven: sửa mọi thứ Phase 2 tìm ra, bất kể độ sâu (spacing → cấu trúc → đổi hướng thị giác). Mọi lần chạy đều mở đầu bằng Aesthetic Customization ở Phase 0, nơi user chọn tường minh 18 tiêu chí phong cách/cấu trúc (typography, màu, motion, shape, layout, tự do cấu trúc, độ táo bạo, và nhiều hơn nữa — xem `references/aesthetic-customization.vi.md`) thay vì để 1 flag âm thầm mở khóa hay kìm hãm sáng tạo. Không còn trần chặn, không còn kiểu "tìm ra rồi nhưng không fix được vì sai mức".
...
2. URL/localhost → **chụp screenshot ngay** bằng bất kỳ screenshot tool nào khả dụng (Playwright MCP, browser tool, v.v.); không có tool nào khả dụng → hỏi user paste screenshot trực tiếp
3. `--pr` → xác định VCS profile theo `~/.claude/skills/_vskills-shared/repo-profile.md` §2 trước (file không tồn tại → coi như full gh mode, đúng hành vi mặc định hiện tại). Full gh mode → `gh pr diff --name-only` để lấy danh sách file. Degraded (thiếu gh / không phải GitHub) → in thông báo §2 và hỏi tên branch, hoặc fallback sang `--diff` (`git diff --name-only`, không cần gh) — rồi tiếp tục vào Phase 1 bình thường.
...
5. Rỗng → hỏi user qua `AskUserQuestion` — **đúng 1 câu**
6. **Aesthetic Customization** — luôn luôn, mọi lần chạy, không cần flag. Trước tiên, hỏi đúng 1 `AskUserQuestion` để chọn chế độ tương tác (nội dung theo `~/.claude/skills/_vskills-shared/webapp-templates.md` mục (c)): **webapp** hay **chat**.
   - **Chat** — đọc toàn bộ `references/aesthetic-customization.vi.md` và chạy đủ 7 vòng `AskUserQuestion` của nó (Vòng 0 tự do cấu trúc + Vòng 1-5 với 17 tiêu chí phong cách — typography, màu, motion, elevation, background, shape, icon, imagery, spacing, grid, layout columns, dark mode, brand tone, độ táo bạo tổng thể, mật độ data/table, style form/input, style empty/loading/error state — cộng câu hỏi vét cuối ở Vòng 6). User chọn tường minh, mỗi lần chạy; agent không bao giờ tự suy đoán hướng đi.
   - **Webapp** — đọc toàn bộ `references/aesthetic-customization.vi.md` để lấy đủ 18 tiêu chí + câu hỏi vét ở Vòng 6 (vẫn tính Recommend-first như cũ cho từng tiêu chí), rồi build 1 JSON template theo `~/.claude/skills/_vskills-shared/webapp-templates.md` mục (a): 18 field `select` (mỗi tiêu chí 1 field, option theo đúng thứ tự Recommend-first) + 1 field `text` cho câu hỏi vét Vòng 6. Dùng `renderHint: "swatch"` cho Triết lý bảng màu và `renderHint: "font-sample"` cho Typography, lấy đúng giá trị hex/font-family đại diện đã ghi kèm mỗi option trong `references/aesthetic-customization.vi.md` — không bao giờ gửi label suông. Health-check + start webapp theo mục (b) nếu chưa chạy, `POST /api/step` (Bash `run_in_background: true`), rồi map kết quả trả về vào đúng 18 quyết định + ghi chú Vòng 6 y hệt như khi hỏi qua chat. Treo/lỗi khi đang chờ → báo ngắn gọn cho user rồi chuyển sang hỏi qua chat cho các tiêu chí còn lại thay vì thử lại hay dừng hẳn skill. Từ bước 7 trở đi không cần biết mode nào đã được dùng.
7. **Domain Research** — có điều kiện, chỉ chạy khi tiêu chí Aesthetic Boldness ở step 6 trả lời *Trend-forward*, *Experimental/bold*, hoặc *Game-inspired/Arcade*, hoặc câu trả lời khác tường minh gọi tới trào lưu thật hiện hành; *Brand-centric* và *Functional/minimalist* bỏ qua thẳng tới step 8:
   ...
   - Tiêu chí 14 = *Game-inspired/Arcade* → ưu tiên nguồn chuyển sang gallery chuyên game UI (Dribbble tag "game UI", GameUIDatabase.com, Behance HUD design) thay vì Awwwards — category Game của Awwwards khá mỏng so với các category thường
   ...
   - Cache check theo từng khía cạnh, độc lập: tìm trong `plans/reports/` file `researcher-vdesign-bold-<slug>-<aspect>*.md` đã tạo trong session/ngày hôm nay — có → tái dùng report của khía cạnh đó; không có → đánh dấu cần research mới
   ...
   - **Nghiên cứu visual/kỹ thuật các nguồn hàng đầu (ở tầng orchestrator, không phải research subagent — subagent không có browser tool):** sau khi Domain Research report về, có browser automation (vd `claude-in-chrome`) khả dụng → mở 1-3 URL nguồn giá trị nhất tìm được và (a) chụp screenshot để cảm nhận tổng thể (bố cục, phối màu, hierarchy — đánh giá mà bullet list không truyền tải nổi) và (b) chạy `getComputedStyle()` qua tool thực thi JS của browser trên 1-2 element nổi bật để lấy giá trị design-token thật, chính xác (box-shadow, border-radius, color, transition-duration/easing) — giá trị computed không bị ảnh hưởng bởi CSS production đã minify/obfuscate, khác với việc đọc stylesheet tải về của site đó, vốn thường không đọc được (class name bị hash, không có cấu trúc) nên không đáng để fetch. Merge các giá trị này vào pattern-pull ở Phase 3. Không có browser automation → bước này bị bỏ qua — report pattern-kèm-URL-nguồn dạng text của Domain Research vẫn là sàn bắt buộc, không phải optional.
   - Khía cạnh nào agent lỗi hoặc không khả dụng: `ui`/`ux`/`animation`/`layout`/`text` fallback im lặng về section catalog tĩnh tương ứng (Cards and Containers / Micro-Interactions / Scroll Animations / Layout and Grids / Typography and Text trong `premium-design-patterns.md` — hoặc, khi tiêu chí 14 là *Game-inspired/Arcade*, section tương ứng trong `references/game-inspired-arcade-patterns.md` thay thế); `3d`/`features`/`flow` không có catalog tĩnh để fallback — vắng mặt hoàn toàn khỏi các bước sau, ghi nhận "unavailable" (khác với "fallback"). Các khía cạnh còn lại (cached, research mới, hoặc fallback) vẫn tiếp tục bình thường; không fail toàn bộ bước domain research chỉ vì 1 khía cạnh lỗi
8. **Design Brief** — trước khi đụng vào code: tổng hợp 18 câu trả lời ở step 6 cộng ghi chú vét ở Vòng 6, cộng kết quả step 7 nếu có chạy, thành 1 brief cụ thể 5-10 dòng (xem section Design Brief trong `references/aesthetic-customization.vi.md`) — tên typeface thật, cách tiếp cận bảng màu thật, timing motion thật, hướng shape/icon/imagery thật, spacing scale thật, hệ thống layout thật, tone thật, mức độ táo bạo thật, và (khi áp dụng) cách xử lý data/table, form, empty/loading/error thật. Một nhãn-một-từ cho mỗi tiêu chí không phải là brief. Nói rõ ra, ghi 18 câu trả lời + Design Brief đã tổng hợp vào `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md` trước khi Phase 1 bắt đầu, nêu trạng thái của step 7 nếu có chạy (cached/fresh/fallback/unavailable/skipped). Phase 3 đọc lại file này trước khi thực thi, thay vì dựa vào trí nhớ hội thoại. Một lần chạy không tổng hợp (18 câu trả lời thô, chưa chốt brief) là lý do lớn nhất khiến output vẫn đọc ra generic/an toàn.
...
**Bước 4 — Component Inventory (chỉ khi target là cả trang/URL có từ 2 cụm component trở lên; bỏ qua với target là 1 component đơn lẻ hoặc PR diff — những target đó giữ nguyên flow Fix cũ, 1 lượt duy nhất ở Phase 3):** liệt kê mọi INSTANCE component hiển thị trên trang thành 1 danh sách đánh số — không chỉ liệt kê theo loại (3 stat card là 3 dòng riêng, không phải 1 mục "stat card"). Gom nhóm instance liền kề nhau trong render tree, cùng vai trò thị giác/cấu trúc, thành cụm (vd 3 stat card xếp hàng = 1 cụm; data table bên dưới = 1 cụm riêng; nav = cụm riêng của nó). Ghi danh sách này, kèm checkbox cho từng instance, vào `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md` dưới 1 section mới `## Component Inventory` — Phase 3 chọn cụm pilot từ đây, Phase 4 verify mọi checkbox đã được xử lý trước khi báo cáo xong.
...
Ghi toàn bộ danh sách findings (item đã check/đã flag, cộng danh sách ứng viên content-density) vào cùng file `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md` (append thêm section `## Audit Findings`) trước khi Phase 3 bắt đầu. Khi Component Inventory ở Phase 1 đã chạy, gắn thêm cho mỗi finding thuộc cụm nào (từ danh sách inventory) để Phase 3 biết cụm nào có nhiều finding nhất.

---

### Phase 3: Fix

**Pilot cụm trước, rồi mới lan rộng (chỉ khi Component Inventory ở Phase 1 đã chạy — target là cả trang/URL có ≥2 cụm). Target là 1 component đơn lẻ hoặc PR diff thì không có Component Inventory — đi thẳng vào "Fix mọi thứ Phase 2 tìm ra" bên dưới, áp dụng 1 lượt cho toàn bộ target, không có bước pilot.**

1. Chọn cụm được gắn nhiều finding nhất từ Phase 2 làm pilot — hòa thì chọn cụm xuất hiện trước trong danh sách Component Inventory.
2. Redesign sâu riêng cụm pilot — toàn bộ nội dung bên dưới trong section Phase 3 này (thực thi Design Brief, boldness pattern, hard rule, anti-slop gate) áp dụng đầy đủ cho cụm này.
3. Trình cụm pilot cho user duyệt trước khi đụng tới bất kỳ component nào khác trong inventory: URL/localhost → chụp screenshot, show before/after; không có → mô tả cách xử lý cụ thể (màu, spacing, motion) và show diff code của riêng cụm pilot. Hỏi qua `AskUserQuestion`: duyệt / chỉnh sửa (user nêu rõ muốn đổi gì) / thử hướng khác.
4. Chỉnh sửa hoặc đổi hướng → làm lại pilot theo feedback đó, trình lại. Cap 3 vòng (tái dùng đúng số review-cycle của vreview) — quá 3 vòng thì hỏi "duyệt kèm bất đồng đã nêu, hay dừng lại".
5. Sau khi duyệt: lan CÙNG ngôn ngữ thiết kế (màu, elevation, radius, spacing scale, motion — không copy y hệt giá trị) cho từng item còn lại trong Component Inventory, tick từng cái khi xong. Component khác loại với pilot vẫn giữ đúng pattern riêng của nó (table vẫn là table, form vẫn là form) chỉ restyle theo ngôn ngữ đó, không ép theo đúng hình dạng của pilot.
6. Cập nhật checkbox Component Inventory trong `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md` khi từng item hoàn tất — Phase 4 verify dựa vào đây.

**Đổi ý giữa chừng cho 1 tiêu chí.** Phase 3 phát hiện 1 câu trả lời tiêu chí ở Phase 0 không còn phù hợp (vd đã chốt *Giữ token hiện có* nhưng giờ có gap thật cần thêm accent màu mới) → đừng restart lại toàn bộ Aesthetic Customization. Hỏi đúng 1 `AskUserQuestion` cho riêng tiêu chí đó (giữ nguyên options, đánh dấu rõ đáp án hiện tại), cập nhật lại dòng tổng hợp của tiêu chí đó trong file Design Brief, tiếp tục Phase 3 từ chỗ đang dừng. Cap 3 lần đổi ý/run (tái dùng đúng cap của bước duyệt pilot cluster ở trên) — quá 3 lần thì hỏi user: khoá Design Brief hiện tại và tiếp tục, hay dừng run để re-plan.

Fix mọi thứ Phase 2 tìm ra — spacing, state, swap component, đổi cấu trúc grid/layout, đổi hoàn toàn hướng thị giác đều nằm trong scope mặc định; không còn trần chặn độ sâu. Độ sâu vẫn phải *tương xứng* với cái thực sự hỏng: 1 finding chỉ về spacing không cho phép viết lại toàn bộ JSX (xem Anti-Patterns — "viết lại toàn bộ layout khi chỉ cần chỉnh một phần"). Điều này áp dụng cả chiều ngược lại — 1 layout thật sự chung chung/lặp lại (mọi section render giống hệt nhau bất kể nó thực sự chứa/làm gì) tự nó là 1 finding ở tầm cấu trúc (View-type fit của Information Architecture, silent-drift check của Proportion & Scale Harmony), không phải hình dạng cần giữ nguyên vì thận trọng; đổi cấu trúc để mỗi section khớp với nội dung thật của nó mới là fix *tương xứng* ở đây, không phải 1 rewrite không ai yêu cầu. Riêng thay đổi cấu trúc/view-type theo đúng câu trả lời Tự Do Cấu Trúc ở Vòng 0: *Giữ nguyên cấu trúc* cấm hoàn toàn dù audit tìm ra gì; *Refine nếu audit phát hiện lệch* giữ nguyên trần "tương xứng" ở trên; *Tự do đổi hẳn* gỡ trần đó, chỉ riêng cho nước đi cấu trúc.

**Thực thi Design Brief.** Design Brief ở step 8 Phase 0 — không phải 1 flag — quyết định lần chạy này táo bạo hay tiết chế đến đâu. Tiêu chí 14 (Aesthetic Boldness) là *Experimental/bold* hoặc *Trend-forward* → art direction riêng biệt, typography scale biểu cảm, layout độc đáo/bất đối xứng, motion tùy chỉnh, điểm nhấn trang trí (icon anchor, khung lồng tông nhạt — xem `references/quiet-polish-patterns.vi.md`), khoảnh khắc chọn/hoàn thành/feedback biểu cảm (xem section Motion & Interaction trong file đó) đều chính đáng — 1 transition thật sự thú vị hay 1 khoảnh khắc hoàn thành đáng ăn mừng không phải "không cần thiết", đó chính là cái được yêu cầu. Tiêu chí 14 là *Brand-centric/cohesive* hoặc *Functional/minimalist* → giữ tiết chế và bám sát hệ thống hiện có, theo 17 câu trả lời còn lại. Tiết chế không có nghĩa là chỉ audit-fix — checklist đầy đủ của Phase 2 (thứ bậc typography, elevation/depth, cân đối tỷ lệ, nội dung dư thừa/lỗi thời) vẫn dẫn dắt Phase 3 trên mọi component trong phạm vi, chỉ là thực thi không kèm các động tác trang trí trong catalog pattern táo bạo; 1 component không có bug chức năng nào vẫn có thể fail các check này và cần restyle. Dù hướng nào: cái vẫn còn là sàn, không phải trần: nguyên tắc usability ở Phase 2 (đủ state bắt buộc, giới hạn số lựa chọn hiển thị theo Hick's Law, confirm chỉ cho action phá hủy) — độ táo bạo vẫn phải vượt qua sàn này chứ không được bỏ qua nó. Cấm hoàn toàn bất kể brief là gì: hard rule kỹ thuật bên dưới (accessibility, tech stack, logic/API, motion GPU-safe) — ràng buộc an toàn/kỹ thuật, không phải lựa chọn thẩm mỹ, không câu trả lời tiêu chí nào đụng vào được.

**Vượt qua ảnh reference, không chỉ khớp nó.** Input có ảnh reference/target (mockup đầy tham vọng, screenshot đối thủ, hoặc concept do AI tạo — xem Input Modes) thay vì chỉ ảnh "before" cần fix → coi đó là mức cần đạt, rồi vượt qua nó ở những trục mà ảnh tĩnh về bản chất không cạnh tranh nổi: nhất quán xuyên suốt mọi màn hình trong cùng flow (không chỉ 1 màn được cho xem), motion/interaction thật (ảnh tĩnh bị đóng băng — xem section Motion & Interaction trong quiet-polish-patterns.vi.md), độ chính xác đo được (đúng lưới spacing/tỉ lệ contrast — mockup tĩnh thường có sai lệch nhỏ chưa đo mà bản triển khai thật không nên lặp lại), và responsive/accessibility thật (mockup chỉ cần đẹp ở 1 kích thước cố định). Khớp các device cụ thể của nó trước (nhóm khung, tầng màu, iconography, nước đi layout), rồi thắng ở những trục này — đừng dừng lại ở mức ngang bằng.
...
**Khi brief đòi táo bạo: một điểm nhấn duy nhất, không rải rác táo bạo.** Chốt MỘT nước đi táo bạo — kỷ luật màu, custom typeface, ngôn ngữ motion, illustration style riêng, hoặc layout bất đối xứng — rồi giữ mọi thứ khác kiềm chế xung quanh nó. Táo bạo nửa vời (mỗi thứ một ít) đọc ra generic HƠN là cam kết trọn vẹn vào một thứ; lý do #1 khiến output táo bạo vẫn đọc ra an toàn trong thực tế. Táo bạo phải mang tính **cấu trúc** (typography scale, contrast, motion-as-clarity, giọng điệu riêng), không bao giờ chỉ là trang trí.
...
**Kéo pattern, không chỉ audit-fix, khi brief đòi táo bạo.** Sau khi chốt Design Brief, kéo từ 6 pattern trên cộng arsenal riêng của brief đó — `~/.claude/skills/frontend-design/references/premium-design-patterns.md` tồn tại → đọc file đó để có catalog đầy đủ (navigation, layout, card, scroll-driven animation, kinetic typography, micro-interaction); không có → dùng archetype pattern trong `references/anti-slop-minimum.vi.md` thay thế. **Luôn đọc thêm `references/quiet-polish-patterns.vi.md`** — 1 tier tiết chế (khung lồng tông nhạt, icon anchor, eyebrow tag, radio-card) cho khi brief cần sự ấm áp/tinh tế hơn là táo bạo; bổ sung thường trực merge thêm vào catalog trên, không phải fallback khi catalog thiếu. **Tiêu chí 14 là *Game-inspired/Arcade* → đọc thêm `references/game-inspired-arcade-patterns.vi.md`** — có điều kiện theo đúng câu trả lời boldness cụ thể đó, khác với tier quiet-polish luôn merge mặc định — cộng thêm pattern/insight domain-specific từ các report domain research nhóm design-pattern ở Phase 0 step 7 hiện có cho lần chạy này (`ui`/`ux`/`animation`/`layout`/`3d`/`text`, tối đa 6 — report `features`/`flow` không nằm trong nhóm này, chỉ để tham khảo cho Design Brief, không phải nguyên liệu kéo pattern). Merge tất cả, không thay thế catalog. Audit ở Phase 2 luôn chạy đầy đủ — bao gồm thứ bậc thị giác, tỷ lệ, elevation, và cân đối nội dung, không chỉ bắt state hỏng/a11y/responsive; khi brief táo bạo nó là sàn, không phải trần, chồng thêm lên phần đó — output được đánh giá bằng độ đặc trưng của điểm nhấn DUY NHẤT đã chốt, không chỉ bằng việc không còn lỗi.
...
- ✅ **Đề xuất content-density không bao giờ tự áp dụng, ở bất kỳ mode nào**: check "sparse/trống trải" ở Information Architecture (Phase 2) đánh dấu có data thật chưa render đáng đưa lên → gộp thành 1 danh sách đề xuất ngắn, cụ thể (tên field + vị trí sẽ đặt — không bao giờ bịa/placeholder), trình qua `AskUserQuestion` ở Phase 4 để user chấp nhận/từ chối từng mục, trước khi thêm gì cả. Quyết định content/product, không phải thẩm mỹ — luôn ở dạng đề xuất bất kể brief.
- ✅ Kiểm tra `package.json` trước khi thêm dependency — trừ dependency allowlist ở trên, thêm thẳng được khi tiêu chí Motion đòi hỏi
...
1. Ban đầu có URL/localhost → chụp screenshot sau khi fix, so sánh before/after
2. Có ảnh input → verify code có khớp ý đồ của ảnh không
3. Chạy `pnpm format` (nếu project có)
4. **Verify accessibility**: project đã có (hoặc user cho phép thêm) `@axe-core/playwright` hoặc `axe-core` thường → chạy một script ngắn nhắm vào route/component đang audit, in ra violations. Không có sẵn → in ra lệnh manual-check tương đương để user tự chạy — review visual/manual thủ công không được là verification duy nhất cho accessibility.
5. Vừa edit trực tiếp một file ảnh dưới `public/` (ghi đè cùng path, tên không đổi) và user báo "không thấy thay đổi" → đừng vội sửa code/ảnh lần nữa; báo user hard-refresh hoặc xóa `.next/cache/images` + restart dev server trước — Next.js Image Optimizer cache theo URL+size, không theo nội dung file, nên fix nhiều khả năng đã đúng nhưng vẫn đang serve bản cache cũ
...
7. **Self-Check** (inline, mọi lần chạy, không cần subagent): trước khi viết báo cáo ngắn, xác nhận — (a) độ sâu fix tương xứng với cái Phase 2 thực sự tìm ra, không có rewrite ngoài yêu cầu; (b) mọi category Phase 2 áp dụng được đã xử lý hoặc đánh dấu rõ not-applicable; (c) anti-slop gate (kể cả illustration check) đã thực sự được kiểm tra, không chỉ ngầm hiểu; (d) kết quả cuối cùng thực sự khớp Design Brief từ Phase 0 step 8, không phải bản tiết chế hơn hay táo bạo hơn brief đó; (e) khi Component Inventory ở Phase 1 đã chạy, mọi item trong đó đã được tick xong trong `plans/reports/vdesign-brief-<slug>-<HHMMSS>.md` hoặc ghi rõ lý do "không áp dụng" — không được bỏ sót component nào một cách âm thầm.
...
- ❌ Đổi cấu trúc page/section khi tiêu chí Tự Do Cấu Trúc ở Vòng 0 chọn *Giữ nguyên cấu trúc* — kể cả khi audit ở Phase 2 nghĩ 1 view-type khác sẽ đẹp hơn
...
- ❌ Bỏ qua Aesthetic Customization ở Phase 0 hoặc coi 18 câu trả lời của nó là chi tiết trang trí thay vì tổng hợp thành Design Brief thật (step 8) trước khi đụng code — 1 lần chạy không có điểm neo vẫn đọc ra là generic/an toàn; chọn pattern trước khi chọn hướng chỉ tạo ra một mớ hổ lốn, không phải một thiết kế mạch lạc
