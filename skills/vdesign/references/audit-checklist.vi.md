...
- [ ] **View-type có phù hợp**: view type đang chọn (table, card grid, list, detail panel, form, kanban board, timeline, masonry/bento grid, split master-detail, calendar/heatmap, comparison matrix, tree/outline, carousel/feed — và nhiều hơn nữa, chọn theo đúng cấu trúc data và task thật sự cần) có thật sự khớp với shape data và task đã nêu ở bước Data & Mục đích của Phase 1 — hay đang ép một cách gượng gạo (dataset quan hệ/lồng nhau nhồi vào bảng phẳng, task 1-bản-ghi bị chôn trong list view)?
- [ ] **Kiểm tra lặp-chung-chung**: các section/card mang nội dung khác nhau về mặt ý nghĩa hoặc phục vụ mục đích khác nhau có đang bị render thành khối chung chung giống hệt nhau chỉ vì lý do đồng nhất thị giác không? Sự chung chung đó TỰ NÓ là 1 finding ở đây, không phải mặc định an toàn — đánh dấu nó, và đổi cấu trúc để mỗi section khớp với nội dung thật của nó, đừng giữ nguyên hình dạng lặp lại vì thận trọng.
- [ ] Có field mà task ở Phase 1 thật sự cần nhưng lại thiếu hoàn toàn khỏi view vì chưa từng được fetch (khác với đã fetch nhưng chưa render — xem mục sparse/trống trải bên dưới)? Đánh dấu là backend dependency trong báo cáo cuối, không bịa ra ở client.
