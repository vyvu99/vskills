...
6. **Aesthetic Customization** — always, every run, no flag. First, ask 1 `AskUserQuestion` to pick the interaction mode (wording per `~/.claude/skills/_vskills-shared/webapp-templates.md` §(c)): **webapp** or **chat**.
   - **Chat** — read `references/aesthetic-customization.md` in full and run its 7 `AskUserQuestion` rounds (Round 0's structural freedom + Rounds 1-5's 17 style criteria — typography, color, motion, elevation, background, shape, icon, imagery, spacing, grid, layout columns, dark mode, brand tone, overall boldness, data/table density, form/input style, empty/loading/error state style — plus Round 6's closing catch-all question). The user picks explicitly, per run; the agent never infers a direction silently.
   - **Webapp** — read `references/aesthetic-customization.md` in full for the same 18 criteria + Round 6 catch-all (same Recommend-first computation per criterion), then build 1 JSON template per `~/.claude/skills/_vskills-shared/webapp-templates.md` §(a): 18 `select` fields (one per criterion, options in Recommend-first order) + 1 `text` field for the Round 6 catch-all. Use `renderHint: "swatch"` for Color palette philosophy and `renderHint: "font-sample"` for Typography, taking the representative hex/font-family values noted next to each option in `references/aesthetic-customization.md` — never send a bare label. Health-check + start the webapp per §(b) if not already running, `POST /api/step` (Bash `run_in_background: true`), then map the returned object back onto the same 18 decisions + Round 6 note the chat flow would have produced. `400` (invalid template — see webapp-templates.md §(b)) → fix the JSON per `details` and retry `/api/step`, still in webapp mode. Timeout/connection error while waiting → tell the user briefly and fall back to the chat flow for the remaining criteria instead of retrying or aborting the skill. Everything from step 7 onward proceeds identically regardless of which mode was used.
